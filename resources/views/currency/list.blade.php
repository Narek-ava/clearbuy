<x-custom-layout>
    ...
    <x-slot name="top">
        <div class="flex items-center">
            <x-common.button.group>
                <x-common.button.a href="/admin/currency?backUrl={{ urlencode($backUrl) }}">New Currency
                </x-common.button.a>
    ...
    <x-list>
        <x-slot name="search">
            ...
            <div class="space-x-2">
                <label>Symbol</label>
                <input type="text" name="symbol" value="{{ Request()->symbol }}" class="border px-2 py-0.5">
            </div>
            <div class="space-x-2">
        ...
        <form class="deleteItemsForm" action="delete_currencies" method="post">
            @csrf
            ...
            <x-common.table.table x-data="tableComponent('{{$sort}}', '{{$order}}')">
                <x-slot name="thead">
                    ...
                    <x-common.table.th>
                        <x-common.sortable sort="{{ $sort }}" order="{{ $order }}" name="symbol" />
                    </x-common.table.th>
                    <x-common.table.th>
                ...
                @foreach ($items as $item)
                    <x-common.table.tr>
                        ...
                        <x-common.table.td>{{ $item->symbol }}</x-common.table.td>
                        <x-common.table.td>
                            {{ $item->countryNames() ? $item->countryNames() : '' }}
                        </x-common.table.td>
                        <x-common.table.td>
                            <x-common.button.group  class="justify-end">
                                <x-common.button.a href="/admin/currency?id={{ $item->id }}&backUrl={{ urlencode($backUrl) }}">
                                    Edit
                                </x-common.button.a>
                            </x-common.button.group>
                        </x-common.table.td>
