    ...
    <x-slot name="top">
        <div class="flex items-center">
            <x-common.button.group>
                <x-common.button.a href="/admin/measure?backUrl={{ urlencode($backUrl) }}">New Measure
                </x-common.button.a>
    ...
    <x-list>
        <x-slot name="search">
            ...
            <div class="space-x-2">
                ...
                <input type="text" name="short_name" value="{{ Request()->short_name }}" class="border px-2 py-0.5">
            </div>
        </x-slot>
        ...
        <form class="deleteItemsForm" action="delete_measures" method="post">
            @csrf
            ...
            <x-common.table.table x-data="tableComponent('{{$sort}}', '{{$order}}')">
                <x-slot name="thead">
                    ...
                    <x-common.table.th>
                        <x-common.sortable sort="{{ $sort }}" order="{{ $order }}" name="short_name">
                            short name
                        </x-common.sortable>
                    </x-common.table.th>
                ...
                @foreach ($items as $item)
                    <x-common.table.tr>
                        ...
                        <x-common.table.td>{{ $item->short_name }}</x-common.table.td>
                        <x-common.table.td>
                            <x-common.button.group  class="justify-end">
                                <x-common.button.a href="/admin/measure?id={{ $item->id }}&backUrl={{ urlencode($backUrl) }}">
                                    Edit
                                </x-common.button.a>
                            </x-common.button.group>
                        </x-common.table.td>
